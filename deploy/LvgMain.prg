...
***            Lvg.exe "config"
***            Lvg.exe "config" "IIS://localhost/w3svc/21/root"
************************************************************************
...
IF VARTYPE(lcAction) = "C" AND StartsWith(LOWER(lcAction),"config")
         do Lvg_ServerConfig.prg with lvParm1
      RETURN 
ENDIF

*** Load the server - wc3DemoServer class below
goWCServer = CREATE("LvgServer")
...
IF llAutoRestart
   LiveReloadShutdown("DO LvgMain.prg")
ELSE
...
**************************************************************
DEFINE CLASS LvgServer AS WWC_SERVER OLEPUBLIC
*************************************************************
...
*** THIS.cAppIniFile = addbs(THIS.cAppStartPath) + "Lvg.ini"
...
THIS.oConfig = CREATEOBJECT("LvgConfig")
...
***************************************************************
DEFINE CLASS LvgConfig AS wwServerConfig
***************************************************************
*: Author: Rick Strahl
...
cHTMLPagePath = "d:\WebConnectionProjects\lvg\Web\"
cDATAPath = ".\"
cVirtualPath = "/lvg/"
